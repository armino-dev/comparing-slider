...
const buildCss = async () => {
    await esbuild.build({
        entryPoints: ['src/scss/comparing-slider.scss'],
        outfile: 'dist/comparing-slider.min.css',
        bundle: true,
        minify: true,
        plugins: [
            sassPlugin({
...
const buildJs = async () => {
    await esbuild.build({
        entryPoints: ['src/comparing-slider.js'],
        bundle: true,
        minify: true,
        metafile: process.env.NODE_ENV !== 'production',
        sourcemap: process.env.NODE_ENV !== 'production',
        outfile: 'dist/comparing-slider.min.js',
        define: {
            'process.env.NODE_ENV': `"${ENVIRONMENT}"`,
        }
    });
